...
const RegionComponent = () => {
  ...
  const indexOfLastRegion = (page + 1) * rowsPerPage;
  const indexOfFirstRegion = indexOfLastRegion - rowsPerPage;
  const currentRegions = countryRegions.slice(
    indexOfFirstRegion,
    indexOfLastRegion,
  );
  ...
  return (
    <div className="dark:text-white p-6">
      <div className="dark:bg-[#252525] bg-white px-5 pt-5 rounded">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left bg-white dark:bg-[#252525] border-separate border-spacing-0 p-2">
            ...
            <tbody className="text-gray-700 dark:text-gray-300">
              {currentRegions.map((region: any, idx: number) => (
        ...
        {/* Pagination */}
        <TablePagination
          totalItems={currentRegions.length}
          currentPage={page}
